...
class AlphaZero:
    ...
    def execute_episode(self):
        ...
        while True:
            ...
            action = np.random.choice(len(policy), p=policy)  # choose an action according to policy
            state, reward, done = env.step(action)  # apply the action to env
            if done:
                # Use the reward from env.step instead of env.get_result
                return [(x[0], x[1], reward * ((-1) ** (x[2] != player))) for x in train_examples]
