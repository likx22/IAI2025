                ...
                if command == 'run':
                    ...
                    for e in range(int(args)):
                        ...
                        while True:
                            ...
                            if done:
                                # record all data
                                z = reward if abs(reward) > 1e-6 else 0
                                for i in range(len(train_examples)):
                                    obs, pi, pl = train_examples[i]
                                    obs_can = env.compute_canonical_form_obs(obs, pl)
                                    train_examples[i] = (obs_can, pi, z if pl == 1 else -z)
                                all_examples.extend(train_examples)
                                all_episode_len.append(episode_step)
                                result_counter.add(z, 1)
                                break
            
                            mcts = mcts.get_subtree(action)  # update mcts
                            if mcts is None:
                                mcts = puct_mcts.PUCTMCTS(env, net, config)
                            ########################
...
if __name__ == "__main__":
    ...
    logger.setLevel(logging.INFO)
    file_handler = logging.FileHandler("log_bs=32_lr=0.0001_wd=0.001.txt")
    ...
    config = AlphaZeroConfig(
        ...
        temperature=1.0, 
        C=1.0,
        checkpoint_path="checkpoint/mlp_7x7_3layers_exfeat_1_0"
    )
    ...
    N_WORKER = 8 # increase this as large as your device can afford
