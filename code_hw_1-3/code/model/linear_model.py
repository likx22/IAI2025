...
class NumpyLinearModel():
    ...
    def loss_v(self, v_pred:np.ndarray, v_trgt:np.ndarray) -> float:
        ...
        ########################
        B=v_trgt.shape[0]    
        return np.sum((v_trgt-v_pred)*(v_trgt-v_pred))/B
    ...
    def grad_v(self, X:np.ndarray, v_trgt:np.ndarray) -> Tuple[np.ndarray, np.ndarray]:
        ...
        a = X @ self.w_v + self.b_v           #(B,)
        v = np.tanh(a)                        #(B,)
    

        act_deriv = 1 - v*v                   #(B,)

        v_trgt=v_trgt.reshape(-1)
        delta = (v_trgt - v) * act_deriv      #(B,)

        grad_w_v = -2 / B * (X.T @ delta)     #(O,)
        grad_b_v = -2 / B * np.sum(delta)     #(1,)
